...
int IMU380::writeRegister(uint8_t address, uint8_t const &data) {
    ...
    _spi->beginTransaction(SPISettings(SPI_SPEED, MSBFIRST, SPI_MODE3,DATA_SIZE_16BIT));
...
int IMU380::readSensor() {
	...
	raw_acceleration.x =_ax;
	raw_acceleration.y =_ay;
	raw_acceleration.z =_az;
...
uint16_t IMU380::getSerialNumber() {
    ...
    // Send address and read data
    _serialNumber =_spi->transfer16(((uint16_t) 0x58) << 8);
...
void IMU380::measure_acceleration() {
